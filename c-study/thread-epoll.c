...
#define EPOLL_NUMBER 128
#define BUFF_MAX_LEN 32768 
...
//服务器参数
static listen_info_t s_listens[LISTEN_MAX];
...
int main(int argc, char *argv[])    //客户端驱动
{
    ...
    {
        printf("init thread pool is success.\n");
    }
#endif
...
#if DEBUG
        printf("Create service is success. #%d\n", __LINE__);
#endif
        s_listens[i].fd = rc;
    }

    //设置集合
    epfd = epoll_create(EPOLL_NUMBER);
    for (i = 0; i < LISTEN_MAX; i++)
    {
        //加入epoll事件集合
        ev.events = EPOLLIN;
        ...
        {
            fprintf(stderr, "向 epoll 集合添加套接字失败(fd = %d), #index = %d\r\n", rc, i);
            exit(-1);
        }
#if DEBUG
        printf("Add events to epoll is success. #%d\n", __LINE__);
#endif
    }
    ...
    for( ; ; ) {
        ...
        for(i = 0; i < nfds; i++)
        {
            ...
            for(j = 0; j < THREAD_MAX; j++)
            {
                ...
                {
#if DEBUG
                    printf("Notice: s_thread_para[%d][0] is 0.\n", j);
#endif
                    break;
                }
            }
            if (j >= THREAD_MAX)
            {
                fprintf(stderr, "[Warning]线程池已满, 连接将被放弃.\r\n");
...
static int init_listen4(char *ip4, int port, int max_link)
{
    ...
    if (0 > bind(sock_listen4, (struct sockaddr *)&addr4, sizeof(addr4)))
    {
        close(sock_listen4);
#if DEBUG
        printf("Bind ip and port is fail...\n");
#endif
        return(-1);
    }
    ...
    if (0 > listen(sock_listen4, max_link))
    {
        close(sock_listen4);
#if DEBUG
        printf("Listen ip and port is fail...\n");
#endif
        return(-1);
    }
...
wait_unlock:
    ...
    //接收请求
    len = recv(sock_cli, buff, BUFF_MAX_LEN, MSG_NOSIGNAL);
#if DEBUG
    if (len)
    {
        printf("[Client info:]\n%s\n", buff);
    ...
    p += sprintf(p, "Content-Type: text/html; charset=UTF-8\r\n");
    ...
    goto wait_unlock;
#if DEBUG
    printf("[Debug]pthread_exit(), destory thread poll.\n");
#endif
    pthread_exit(NULL);
}
